...
void MixColumns_Xtime_Optimize(unsigned char* state)
{
    ...
    {
        //? 02 03 01 01
        src[0] = sbox2[state[4 * cnt_i]]^sbox3[state[4 * cnt_i+1]] ^ state[4 * cnt_i + 2] ^ state[4 * cnt_i + 3];                                                   // 최종 src
        //? 01 02 03 01
        src[1] = sbox2[state[4 * cnt_i + 1]]^sbox3[state[4 * cnt_i + 2]] ^ state[4 * cnt_i] ^ state[4 * cnt_i+3];                                                   // 최종 src
        //? 01 01 02 03
        src[2] = sbox2[state[4 * cnt_i + 2]]^sbox3[state[4 * cnt_i + 3]] ^ state[4 * cnt_i] ^ state[4 * cnt_i+1];                                                   // 최종 src
        //? 03 01 01 02
        src[3] = sbox2[state[4 * cnt_i + 3]]^sbox3[state[4 * cnt_i]] ^ state[4 * cnt_i+1] ^ state[4 * cnt_i+2];
